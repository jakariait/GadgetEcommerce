...
const MenuItem = memo(({ label, children }) => {
  ...
  return (
    ...
    >
      <div className="px-2 py-2 text-black hover:secondaryTextColor font-semibold cursor-pointer">
        {label}
      </div>
...
const SubMenuItem = memo(
  ({ subCategory, childCategories, buildQueryString, isCurrentRoute }) => {
    ...
    const hasChildCategories =
      ...
      childCategories.some(
        (child) =>
          String(child?.subCategory?._id || child?.subCategory) ===
            String(subCategory._id) && child.isActive,
      );

    const handleMouseEnter = () => {
      if (timerRef.current) clearTimeout(timerRef.current);
      if (hasChildCategories) setIsChildOpen(true);
    };

    const handleMouseLeave = () => {
      timerRef.current = setTimeout(() => setIsChildOpen(false), 200);
    };
    ...
    return (
      ...
      >
        <Link
          to={subCategoryPath}
          className="block w-full text-left hover:secondaryTextColor p-1 rounded transition-colors duration-150"
          onClick={(e) => {
        ...
        {hasChildCategories && (
          <div
            className={`absolute left-full top-0 mt-[-32px] w-56 bg-white shadow-lg rounded-md z-10 transition-all duration-200 ${
              isChildOpen
            ...
            return (
              <li key={childCategory._id} className="px-2 py-1">
                <Link
                  ...
                  className="text-left w-full hover:secondaryTextColor p-1 rounded text-sm block transition-colors duration-150"
                  onClick={(e) => {
