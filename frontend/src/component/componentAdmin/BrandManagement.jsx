...
import { createBrand, getBrands, updateBrand, deleteBrand } from '../../api/brandApi';
import {
  Dialog,
  DialogActions,
  DialogContent,
...
const BrandManagementContent = () => {
  ...
  const columns = [
    {
      name: 'Logo',
      cell: (row) => (
        <img src={`/uploads/${row.logo}`} alt={row.name} className="w-16 h-16 object-contain" />
      ),
    },
    {
      name: 'Name',
    ...
    },
    {
      ...
      cell: (row) => (
        <div>
          <Button variant="contained" color="primary" onClick={() => openModal(row)} style={{ marginRight: '8px' }}>Edit</Button>
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <Button variant="contained" color="primary" onClick={() => openModal()} className="mb-4">
        Add Brand
      </Button>
